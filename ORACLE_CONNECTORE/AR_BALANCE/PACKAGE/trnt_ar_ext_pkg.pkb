DROP PACKAGE BODY APPS.TRNT_xxx_AR_EXT_PKG;

CREATE OR REPLACE PACKAGE BODY APPS.trnt_xxx_ar_ext_pkg AS
/* $Header: trnt_ar_ext_pkg.pkg, Version 1.0, 09-JAN-2020 $
***********************************************************************
* *
* History Log *
* *
...
**********************************************************************/
    PROCEDURE trnt_ar_ext_file (
        ...
        l_period_num          VARCHAR(200);
        hd_flag               NUMBER := 1;

        /*Declaration of variables for Accounts Receivable Extract API*/
        ...
        v_result              CLOB;
        l_coa_id              NUMBER;
        CURSOR cur_cad_ar_bal_ext IS
        
        select
            decode(l_seg1, 'Y', cc.segment1, '')  segment1,
            decode(l_seg2, 'Y', cc.segment2, '')  segment2,
            decode(l_seg3, 'Y', cc.segment3, '')  segment3,
            decode(l_seg4, 'Y', cc.segment4, '')  segment4,
            decode(l_seg5, 'Y', cc.segment5, '')  segment5,
            decode(l_seg6, 'Y', cc.segment6, '')  segment6,
            decode(l_seg7, 'Y', cc.segment7, '')  segment7,
            decode(l_seg7, 'Y', cc.segment8, '')  segment8,
            decode(l_seg7, 'Y', cc.segment9, '')  segment9,
            decode(l_account_seg, 'SEGMENT1', cc.segment1, 'SEGMENT2', cc.segment2,
                   'SEGMENT3', cc.segment3, 'SEGMENT4', cc.segment4, 'SEGMENT5',
                   cc.segment5, '') account_no,
            ...
            ps.invoice_currency_code trans_curr_code, 
            sum(rgld.acctd_amount) sum_trans_amount ,
            sum(rgld.amount) local_trans_amount
from 
       ...
       gl_ledgers                    gll
where 
        rta.customer_trx_id = ps.customer_trx_id
        AND rta.customer_trx_id = rgld.customer_trx_id
        AND rgld.code_combination_id = cc.code_combination_id
        AND ps.customer_id = cust_acct.cust_account_id
       -- AND ps.customer_id=1007
        and cc.CHART_OF_ACCOUNTS_ID=l_coa_id   --5369848.06
        and rgld.account_class in ('REV','TAX','ROUND','SUSPENSE','FREIGHT')
        and ps.gl_date BETWEEN '1-JAN-1950' AND l_period_end_date
        and gll.CHART_OF_ACCOUNTS_ID =cc.CHART_OF_ACCOUNTS_ID
        --and ps.org_id=204    
        --and class='INV'  
        and gll.ledger_id=l_ledger_id
        AND cc.concatenated_segments IN (
                            SELECT
...
group by
            decode(l_seg1, 'Y', cc.segment1, ''),
            decode(l_seg2, 'Y', cc.segment2, ''),
            decode(l_seg3, 'Y', cc.segment3, ''),
            decode(l_seg4, 'Y', cc.segment4, ''),
            decode(l_seg5, 'Y', cc.segment5, ''),
            decode(l_seg6, 'Y', cc.segment6, '') ,
            decode(l_seg7, 'Y', cc.segment7, ''),
            decode(l_seg7, 'Y', cc.segment8, '') ,
            decode(l_seg7, 'Y', cc.segment9, ''),
            decode(l_account_seg, 'SEGMENT1', cc.segment1, 'SEGMENT2', cc.segment2,
                   'SEGMENT3', cc.segment3, 'SEGMENT4', cc.segment4, 'SEGMENT5',
                   cc.segment5, '') ,
            decode(l_account_seg, 'SEGMENT1', 1, 'SEGMENT2', 2,
    ...
    BEGIN
        fnd_file.put_line(fnd_file.log, 'package_name:trnt_ar_ext_pkg,' || 'procedure_name:trnt_ar_ext_file');
        fnd_file.put_line(fnd_file.log, 'p_batch_id:'
                                        || fnd_global.conc_request_id
                                        || ',p_config_id:'
                                        || p_config_id);

        fnd_file.put_line(fnd_file.log, 'AR ' || 'Start AR balance extract ');
        OPEN c_config;
        ...
        fnd_file.put_line(fnd_file.log, 'AR '
        ...
        fnd_file.put_line(fnd_file.log, 'AR ACCOUNT-||Getting segment Details' || l_account_key);
       ...
       ---Year and  period Selection
        BEGIN
            ...
            IF l_period_end_date IS NULL THEN
                fnd_file.put_line(fnd_file.log, 'Error Code: '
                                                || 'ARBALS_PERIOD_END_DATE_NOT_FOUND: '
        ...
        EXCEPTION
            WHEN OTHERS THEN
                fnd_file.put_line(fnd_file.log, 'Error Code:'
                                                || 'ARBALS_PERIOD_DETAILS_NOT_FOUND: Please enter either Period year and period name or Cutoff Period'
        ...
        BEGIN
            ...
            IF l_global_code IS NULL THEN
                fnd_file.put_line(fnd_file.log, 'Error Code: '
                                                || 'ARBALS_GLOBAL_CURRENCY_CODE_NOT_FOUND: '
            ...
            IF local_curr_code IS NULL THEN
                fnd_file.put_line(fnd_file.log, 'Error Code: '
                                                || 'ARBALS_LOCAL_CURRENCY_CODE_NOT_FOUND: '
            ...
            ELSE
                ...
                IF conver_rate_col IS NULL THEN
                    fnd_file.put_line(fnd_file.log, 'Error Code: '
                                                    || 'ARBALS_CONVERSION_RATE_NOT_FOUND: '
      ...
      --SERVER/Report and file path setting--
        IF l_output_type IN (
            ...
            IF l_file_path IS NULL THEN
                retcode := 1;
                fnd_file.put_line(fnd_file.log, 'AR-Please enter the server file output path');
            END IF;
         --deriving db name for the filepath--
            ...
            fnd_file.put_line(fnd_file.log, 'AR balance extractor File path name : '
                                            || l_file_path
                                            || '\'
                                            || v_filename);
       ...
       -- End of SERVER/Report and file path setting--
        ...
        fnd_file.put_line(fnd_file.log, 'segments Setting the header for the extract');
        CLOSE c_seg_sel;
        ...
        fnd_file.put_line(fnd_file.log, 'end of header selection');
         --Setting the records for the extract--         
        FOR rec IN cur_cad_ar_bal_ext LOOP
            IF hd_flag = 1 THEN
                IF l_output_type IN (
                    'S',
                    'X'
        ...
        --End of Setting the header for the extract--
            ...
            IF rec.sum_trans_amount <> 0 THEN
                l_count := l_count + 1;
                BEGIN
                    l_gbl_amount := 0;
           ...
           --Creating record from Query--
                ...
                l_data_line := l_data_line
                               || l_global_code
                               || chr(9)
                               || to_char(round(l_gbl_amount, 3), '9999999999.00')
                               || chr(9)
                               || ( rec.local_curr_code )
                               || chr(9)
                               || to_char(round((rec.local_trans_amount), 3), '9999999999.00')
                               || chr(9)
                               || ( rec.trans_curr_code )
                               || chr(9)
                               || to_char((rec.sum_trans_amount), '9999999999.00')
                               || chr(13);  

        /*Accounts Receivable Extract API */
        ...
        IF l_output_type IN (
            ...
            BEGIN
                ...
                v_jdata := trnt_common_util.get_jsonformat_for_uac(txtfile_tab, 'CAD', 'ARBALS', 'EBS_APPS', rec_config.config_code,
        ...
        IF l_output_type IN (
            ...
            BEGIN
                fnd_file.put_line(fnd_file.log, 'json extract');
                v_jdata := trnt_common_util.get_jsonformat_for_uac(txtfile_tab, 'CAD', 'ARBALS', 'EBS_APPS', rec_config.config_code,
                                        rec_config.enable_encryption, p_config_id, rec_config.config_type, NULL, NULL);
            ...
            fnd_file.put_line(fnd_file.log, 'setting up the file path'); 
            l_directory_name := trnt_common_util.get_dir_name(l_file_path);
            IF l_directory_name IS NULL THEN
                fnd_file.put_line(fnd_file.log, 'No Oracle Directory available for the file path');
                return;
        ...
        fnd_file.put_line(fnd_file.log, 'Writing the extract informations');
        IF l_count = 0 THEN
        ...
        fnd_file.put_line(fnd_file.log, 'ARBAL  END AR balance extract ');
    EXCEPTION
        ...
        WHEN fiscal_year_exception THEN
            fnd_file.put_line(fnd_file.log, 'AR_BAL ERROR fiscal_year_exception Procedure TRNT_ar_BAL_EXTRACT');
            retcode := 1;
            return;
        WHEN OTHERS THEN
            fnd_file.put_line(fnd_file.log, 'Unexpected Error in Procedure TRNT_ar_BAL_EXTRACT'
                                            || ' - '
                                            || sqlerrm
                                            || dbms_utility.format_error_backtrace);
