...
**********************************************************************/
    PROCEDURE trnt_ap_ext_file (
        ...
        l_coa_id              NUMBER;
        l_as_of_date          DATE;
        l_journal_source      varchar2(200);
        l_DEFINITION_CODE     varchar2(50);
        ...
        CURSOR cur_cad_ap_bal_ext IS
          ...
          SELECT DECODE (l_seg1, 'Y', glc.segment1, '')    segment1,
                 ...
                 gl.name                                   ledger_name,
                 sum(tb.ACCTD_ROUNDED_REM_AMOUNT)          local_trans_amount,
                 tb.trx_currency_code                      trans_curr_code,               
                 sum(tb.ENTERED_ROUNDED_REM_AMOUNT)  sum_trans_amount
                  ...
                  */
                 --tb.third_party_name VENDOR_NAME,
                -- tb.third_party_number VENDOR_ID

            FROM xla_trial_balances_gt      tb,
        ...
        cursor c_ledger_set is
          select distinct ledger_id,ledger_name from GL_LEDGER_SET_NORM_ASSIGN_V
          where ledger_set_id=nvl(l_ledger_set,ledger_set_id)
          and ledger_id=nvl(l_ledger,ledger_id);

        rec_ledger_set        c_ledger_set%rowtype;

            */

        v_segment_from1 varchar2(1000);
        v_segment_from2 varchar2(1000);
        v_segment_from3 varchar2(1000);
        v_segment_from4 varchar2(1000);
        v_segment_from5 varchar2(1000);
        v_segment_from6 varchar2(1000);
        v_segment_from7 varchar2(1000);
        v_segment_from8 varchar2(1000);
        v_segment_from9 varchar2(1000);
    ...
    BEGIN
        ...
        --l_ledger := rec_config.ledger;        
        --l_ledger_set := rec_config.ledger_set;        
        --trnt_common_util.create_segments_filter(p_config_id);
        l_ledger_id :=rec_config.ledger;  ---commentfor ledger_set
...
/*
    open c_ledger_set;
        loop
        fetch c_ledger_set into rec_ledger_set;
        exit when c_ledger_set%notfound;
        l_ledger_id := rec_ledger_set.ledger_id;

        fnd_file.put_line(fnd_file.log, 'AP, Ledger- '||rec_ledger_set.ledger_name);
*/
        fnd_file.put_line(fnd_file.log, 'AP, ACCOUNT- ||Getting segment Details');
        l_account_seg := trnt_common_util.get_segment('ACCOUNT', l_ledger_id, rec_config.chart_of_accounts_id, rec_config.config_type);
       ...
       /*Year and  period Selection*/
        ...
        into l_journal_source 
        from GL_JE_SOURCES A WHERE EXISTS 
            (SELECT 1 
    ...
    exception when others then
        fnd_file.put_line(fnd_file.log,'journal source not found'||sqlerrm);
        end;
        ...
        where ledger_id=l_ledger_id;--01
        exception when others then
        fnd_file.put_line(fnd_file.log,'definition code not found'||sqlerrm);
        end;
      -------------------------------
      fnd_file.put_line(fnd_file.log,'l_DEFINITION_CODE'||l_DEFINITION_CODE);
      fnd_file.put_line(fnd_file.log,'l_journal_source'||l_journal_source);
      --------------------------------
    execute immediate 'truncate table xla.xla_trial_balances_gt';
      if l_journal_source is not null and l_DEFINITION_CODE is not null
      then
        DECLARE
            lv BOOLEAN;
        BEGIN

            xla_tb_ap_report_pvt.p_journal_source := l_journal_source;--'Payables';
            xla_tb_ap_report_pvt.p_definition_code :=l_DEFINITION_CODE;--'AP_200_1';
            xla_tb_ap_report_pvt.p_start_date := '01-JAN-1950';
            xla_tb_ap_report_pvt.p_as_of_date := l_period_end_date; --'31-JAN-2010';
            xla_tb_ap_report_pvt.p_show_trx_detail_flag := 'Y';
            xla_tb_ap_report_pvt.p_incl_write_off_flag := 'Y';
            xla_tb_ap_report_pvt.p_security_flag := 'Y';
            xla_tb_ap_report_pvt.p_coa_id := l_coa_id; --'101';
            lv := xla_tb_ap_report_pvt.before_report;
        END;
        end if;
      ...
      /*SERVER/Report and file path setting*/
       ...
       -- End of SERVER/Report and file path setting--
        begin
        select LISTAGG(segment_set_name||'('||segment1_from||'-'||segment1_to||'-'||segment1_Exclude||')',chr(13)) WITHIN GROUP(
                         ORDER BY
                            ROWID
                    ) segment1_from,                  
                    LISTAGG(segment_set_name||'('||segment2_from||'-'||segment2_to||'-'||segment2_Exclude||')',chr(13)) WITHIN GROUP(
                         ORDER BY
                            ROWID
                    ) segment2_from,
                    LISTAGG(segment_set_name||'('||segment3_from||'-'||segment3_to||'-'||segment3_Exclude||')',chr(13)) WITHIN GROUP(
                         ORDER BY
                            ROWID
                    ) segment3_from,
                    LISTAGG(segment_set_name||'('||segment4_from||'-'||segment4_to||'-'||segment4_Exclude||')',chr(13)) WITHIN GROUP(
                         ORDER BY
                            ROWID
                    ) segment4_from,
                   LISTAGG(segment_set_name||'('||segment5_from||'-'||segment5_to||'-'||segment5_Exclude||')',chr(13)) WITHIN GROUP(
                         ORDER BY
                            ROWID
                    ) segment5_from,
                    LISTAGG(segment_set_name||'('||segment6_from||'-'||segment6_to||'-'||segment6_Exclude||')',chr(13)) WITHIN GROUP(
                         ORDER BY
                            ROWID
                    ) segment6_from,
                    LISTAGG(segment_set_name||'('||segment7_from||'-'||segment7_to||'-'||segment7_Exclude||')',chr(13)) WITHIN GROUP(
                         ORDER BY
                            ROWID
                    ) segment7_from,
                    LISTAGG(segment_set_name||'('||segment8_from||'-'||segment8_to||'-'||segment8_Exclude||')',chr(13)) WITHIN GROUP(
                         ORDER BY
                            ROWID
                    ) segment8_from,
                    LISTAGG(segment_set_name||'('||segment9_from||'-'||segment9_to||'-'||segment9_Exclude||')',chr(13)) WITHIN GROUP(
                         ORDER BY
                            ROWID
                    ) segment9_from


        into v_segment_from1,v_segment_from2,v_segment_from3,v_segment_from4,v_segment_from5,v_segment_from6,v_segment_from7,v_segment_from8,v_segment_from9

        from trnt_config_segment 
        where config_id=p_config_id;
        exception when others then
        fnd_file.put_line(fnd_file.log,'segment selection to log err');
        end;
        ...
        fnd_file.put_line(fnd_file.log, 'Selected Segments');
        ...
        LOOP
            ...
            IF rec_seg_sel.segment_name = 'SEGMENT1' THEN
                l_seg1 := 'Y';
                fnd_file.put_line(fnd_file.log,'SEGMENT1: '||rec_seg_sel.SEGMENT_DESC||chr(13)||v_segment_from1);
            ELSIF rec_seg_sel.segment_name = 'SEGMENT2' THEN
                l_seg2 := 'Y';
                fnd_file.put_line(fnd_file.log,'SEGMENT2: '||rec_seg_sel.SEGMENT_DESC||chr(13)||v_segment_from2);
            ELSIF rec_seg_sel.segment_name = 'SEGMENT3' THEN
                l_seg3 := 'Y';
                fnd_file.put_line(fnd_file.log,'SEGMENT3: '||rec_seg_sel.SEGMENT_DESC||chr(13)||v_segment_from3);
            ELSIF rec_seg_sel.segment_name = 'SEGMENT4' THEN
                l_seg4 := 'Y';
                fnd_file.put_line(fnd_file.log,'SEGMENT4: '||rec_seg_sel.SEGMENT_DESC||chr(13)||v_segment_from4);
            ELSIF rec_seg_sel.segment_name = 'SEGMENT5' THEN
                l_seg5 := 'Y';
                fnd_file.put_line(fnd_file.log,'SEGMENT5: '||rec_seg_sel.SEGMENT_DESC||chr(13)||v_segment_from5);
            ELSIF rec_seg_sel.segment_name = 'SEGMENT6' THEN
                l_seg6 := 'Y';
                fnd_file.put_line(fnd_file.log,'SEGMENT6: '||rec_seg_sel.SEGMENT_DESC||chr(13)||v_segment_from6);
            ELSIF rec_seg_sel.segment_name = 'SEGMENT7' THEN
                l_seg7 := 'Y';
                fnd_file.put_line(fnd_file.log,'SEGMENT7: '||rec_seg_sel.SEGMENT_DESC||chr(13)||v_segment_from7);
            ELSIF rec_seg_sel.segment_name = 'SEGMENT8' THEN
                l_seg8 := 'Y';
                fnd_file.put_line(fnd_file.log,'SEGMENT8: '||rec_seg_sel.SEGMENT_DESC||chr(13)||v_segment_from8);
            ELSIF rec_seg_sel.segment_name = 'SEGMENT9' THEN
                l_seg9 := 'Y';
                fnd_file.put_line(fnd_file.log,'SEGMENT9: '||rec_seg_sel.SEGMENT_DESC||chr(13)||v_segment_from9);
            END IF;
